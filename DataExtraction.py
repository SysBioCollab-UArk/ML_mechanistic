...
for CellKeyNum in range(18):
    ...
    vh = popt[2]
    vEo = 1
    pltLogConc = np.linspace(LogConc[0], LogConc[-1], 100)
    vEi = func(pltLogConc, vEmax, vEC, vh)
    # vEi = vEmax + (vEo - vEmax) / (1 + (10 ** LogConc / vEC) ** vh)
    # vEi = (vEC ** vh) / (vEC ** vh + (10 ** LogConc) ** vh)
    # plt.plot(LogConc, vEi, "-", lw=2)
    plt.plot(pltLogConc, vEi, "-", lw=2)
    plt.xlabel(r'log$_{10}$ conc')
    plt.ylabel("relative effect")
    plt.title(Dname + ", " + keys_list[CellKeyNum])
    plt.tight_layout()
    plt.savefig('data/plot_%d.pdf' % CellKeyNum, format='pdf')
